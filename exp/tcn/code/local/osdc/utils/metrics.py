...
class BinaryMeter(object):
    ...
    def get_fa(self):
        return self.fp / (self.get_positive_examples() + np.finfo(float).eps)

    def get_tp(self):
        return self.tp / (self.get_positive_examples() + np.finfo(float).eps)

    def get_tn(self):
        return self.tn / (self.get_positive_examples() + np.finfo(float).eps)

    def get_miss(self):
        return self.fn / (self.get_positive_examples() + np.finfo(float).eps)

    def get_accuracy(self):
        return (self.tp + self.tn) / (self.tp + self.tn + self.fp + self.fn)

    def get_precision(self):
        return self.tp / (self.tp + self.fp + np.finfo(float).eps)

    def get_recall(self):
        return self.tp / (self.tp + self.fn + np.finfo(float).eps)

    def get_f1(self):
        return (2.0 * self.tp) / (2.0 * self.tp + self.fp + self.fn)

    def get_der(self):
        return (self.fp + self.fn) / (self.fn + self.tp + np.finfo(float).eps)

    def get_mattcorr(self):
        return (self.tp * self.tn - self.fp * self.fn) / \
               (np.finfo(float).eps + (self.tp + self.fp) * (self.tp + self.fn) * (self.tn + self.fp) * (
...
class MultiMeter(object):
    ...
    def get_fa(self):
        ...
        for i in range(self.n_classes):
            fa.append(self.fp[i] / (self.get_positive_examples() + np.finfo(float).eps))
    ...
    def get_miss(self):
        ...
        for i in range(self.n_classes):
            ...
            miss.append(self.fn[i] / (self.get_positive_examples() + np.finfo(float).eps))
    ...
    def get_precision(self):
        ...
        prec = []
        for i in range(self.n_classes):
            prec.append(self.tp[i] / (self.tp[i]+ self.fp[i] + np.finfo(float).eps))
    ...
    def get_recall(self):
        ...
        recall = []
        for i in range(self.n_classes):
            recall.append(self.tp[i] / (self.tp[i]+ self.fn[i] + np.finfo(float).eps))
    ...
    def get_der(self):
        ...
        for i in range(self.n_classes):
            der.append( (self.fp[i] + self.fn[i]) / (self.fn[i] + self.tp[i] + np.finfo(float).eps))
    ...
    def get_matt(self):
        ...
        for i in range(self.n_classes):
            matt.append((self.tp[i] * self.tn[i] - self.fp[i] * self.fn[i]) / \
               (np.finfo(float).eps + (self.tp[i] + self.fp[i]) * (self.tp[i] + self.fn[i]) * (self.tn[i] + self.fp[i]) * (
